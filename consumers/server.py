...
def run_server():
    ...
    if topic_check.topic_exists("TURNSTILE_SUMMARY") is False:
        ...
        exit(1)
    if topic_check.topic_exists("com.udacity.johnson.cta.stations.table.v1") is False:
        logger.fatal(
    ...
    consumers = [
        KafkaConsumer(
            "com.udacity.johnson.cta.weather.v1",
            weather_model.process_message,
            offset_earliest=True,
        ),
        KafkaConsumer(
            "com.udacity.johnson.cta.stations.table.v1",
            lines.process_message,
            offset_earliest=True,
            is_avro=False,
        ),
        KafkaConsumer(
            "^com.udacity.johnson.cta.station.arrivals.v1.*",
            lines.process_message,
            offset_earliest=True,
        ),
        KafkaConsumer(
