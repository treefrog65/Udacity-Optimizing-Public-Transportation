...
class Line:
    ...
    def process_message(self, message):
        ...
        if message.topic() == "com.udacity.johnson.cta.stations.table.v1":
            try:
                value = json.loads(message.value())
                self._handle_station(value)
                logger.info(f'Added station {value["station_name"]} ({value["station_id"]}) to line {value["line"]}')
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif message.topic().startswith("com.udacity.johnson.cta.station.arrivals.v1"):
